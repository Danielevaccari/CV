...
const Project: React.FunctionComponent<IProject> = props => {
  ...
  const styleStringModify = (): void => {
    if (hovered && row.current !== null) {
      row.current.style.transform = 'perspective(500px) scale(1.08) rotateX(' + -20 * ((y - height / 2) / height) + 'deg) rotateY(' + 20 * ((x - width / 2) / width) + 'deg)';
    }
  };

  const resetTilting = (): void => {
    if(row.current){
      row.current.style.transform = 'perspective(500px) scale(1) rotateX(0deg) rotateY(0deg) skewy(-5deg)';
    }
  };
  ...
  return (
    <div className='personal-projects-row'>
      ...
      <div className='personal-projects-row__right'>
        <a style={{ textDecoration: 'none' }} href={props.href} className='jj'>
          <img
            id={props.id}
